...
export function RecentCoverCard({ cover }: RecentCoverCardProps) {
      ...
      <CardContent className="p-4">
        ...
        <div className="flex items-center gap-3 mt-3 pt-3 border-t border-border/50 text-xs text-muted-foreground">
          <div className="flex items-center gap-1">
            ...
            <span>{formatRelativeDate(cover.created_at)}</span>
          </div>
          {cover.submitted_by && (
            <div className="flex items-center gap-1">
              <User className="w-3.5 h-3.5" />
              <span>{cover.submitted_by.username}</span>
            </div>
          )}
        </div>
      </CardContent>
